...
//        ArrayList<String> commands = mixture.getValue();
        ...
        try {
            ...
            switch (_string_switcher_2) {
                case "add": {
                    ...
                    else n = new Notification(new NotificationContent(rawContent), false, getTime(), "N/A");
    ...
    public static void version(ArrayList<String> commands){
        ...
        Version[] arr = UpdateInfo.UpdateInfo;
        switch (subCommand){
            case "latest":
                formatter(true,new Mixture<>(arr[arr.length-1].toString(),FontStyle.plainStyle));
                break;
            case "list":
                StringBuilder content = new StringBuilder("DH Versions: ");
                for (Version ver : arr) { content.append("\n").append(ver.head()); }
                formatter(true, new Mixture<>(content.toString(), FontStyle.plainStyle));
                break;
            case "show":
                String ver = commands.get(2);
                formatter(true,new Mixture<>(arr[Integer.parseInt(ver)-1].toString(),FontStyle.plainStyle));
                break;
        }
    }
