...
public class Logger {
    ...
    public void pre(Object message){ format(message.toString());}
    ...
    public void record(Object message, Object... args) {
        ...
        boolean simple = ConfigHandler.CONFIG.isSimpleLoggerOutput();
        String in1 = className;
        if (simple) {
            int third = className.indexOf('.', className.indexOf('.', className.indexOf('.') + 1) + 1);
            if (third != -1) {
                String in2 = className.substring(third + 1);
                if (in2.split("\\.").length > 3) in1 = className.substring(third + 1);
                else in1 = in2;
                in1 = ".../" + in1;
            }
        }
        output = "method:" + in1 + "." + methodName + "(" + (className.substring(className.lastIndexOf(".") + 1)) + ".java:" + lineNumber + ")";
